...
import { orthopedicService } from "@/services/orthopedicService";
import type {
  OrthopedicBeneficiariesResponse,
  OrthopedicConfirmContractResponse,
} from "@/types/api";
...
export const Ortopedico: React.FC = () => {
  ...
  const [orthopedicData, setOrthopedicData] =
    useState<OrthopedicBeneficiariesResponse | null>(null);
  ...
  const handleContinueToVerification = async () => {
    ...
    try {
      await orthopedicService.acceptOrthopedicTerms({
        perfilAutenticado: user.perfilAutenticado,
        codigoMatriculasBeneficiariosAlvo: selectedBeneficiaries,
      });

      await orthopedicService.sendOrthopedicSMS({
        perfilAutenticado: user.perfilAutenticado,
        codigoMatriculasBeneficiariosAlvo: selectedBeneficiaries,
      });
  ...
  const handleConfirmContract = async () => {
    ...
    setIsSubmitting(true);
    try {
      const response = await orthopedicService.confirmOrthopedicContract({
        perfilAutenticado: user.perfilAutenticado,
  ...
  const totalCost = useMemo(() => {
    if (!orthopedicData) return 0;
    return selectedBeneficiaries.length * orthopedicData.valorParaContratacao;
  }, [selectedBeneficiaries, orthopedicData]);
  ...
  return (
    <div className="space-y-8">
      ...
      {step === "beneficiaries" && orthopedicData && (
        <Card className="card-medical">
          <CardHeader>
            <CardTitle>Selecione os Beneficiários</CardTitle>
            <CardDescription>
              Escolha para quem deseja contratar o plano ortopédico.
            </CardDescription>
          </CardHeader>
          <CardContent className="space-y-6">
            <div className="space-y-4">
              {orthopedicData.beneficiarios.map((beneficiary) => (
                <div
            ...
            <Alert>
              ...
              <AlertDescription className="space-y-2">
                <p>
                  ...
                  <b>
                    {orthopedicData.valorParaContratacao.toLocaleString(
                      "pt-BR",
                      {
                        style: "currency",
                        currency: "BRL",
                ...
                <p>
                  ...
                  <b>
                    {new Date(
                      orthopedicData.dataInicioVigenciaAposContratacao
                    ).toLocaleDateString("pt-BR")}
                  </b>
                </p>
                <div className="flex space-x-4">
                  <a
                    href={orthopedicData.linkTermoAdesao}
                    target="_blank"
                  ...
                  <a
                    href={orthopedicData.linkContrato}
                    target="_blank"
                    rel="noopener noreferrer"
                    className="text-primary hover:underline"
                  >
      ...
      {step === "success" && confirmationData && (
        <Card className="card-medical max-w-lg mx-auto">
          <CardHeader className="items-center text-center">
            ...
            <CardDescription>
              O Plano Ortopédico foi contratado com sucesso.
            </CardDescription>
          </CardHeader>
